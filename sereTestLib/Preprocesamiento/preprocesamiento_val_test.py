...
def preprocesamiento_val_test(sample_id):
    """
    ...
    ## El parámetro <<dir_out_ov_split_test>> va a ser la ruta donde voy a guardar la salida de las muestras solapadas correspondientes al paciente que se pasó como entrada.
    ## Para el caso de MI COMPUTADORA <<dir_out_ov_split_test>> = 'C:/Yo/Tesis/sereData/sereData/Dataset/overlaps/test'
    ...
    ## El parametro <<dir_out_ov_split_rename_test>> va a tener la ruta a donde están los datos para todos los pacientes luego de hacer el procesado previo
    ## Para el caso de MI COMPUTADORA <<dir_out_ov_split_rename_>> = 'C:/Yo/Tesis/sereData/sereData/Dataset/rename_overlaps/test'
    ## El parámetro <<directorio_muestra>> me va a contener la carpeta correspondiente al paciente analizado y es el que se arma en las líneas de arriba
    ## Por ejemplo, si se está analizando una muestra corta (<<long_sample>> = False) del paciente de ID 215 (<<sample_ID>> = 215)
    ## El parámetro <<directorio_scalogramas_test>> va a tener la ruta donde voy a guardar los escalogramas a la salida correspondientes a los segmentos procesados.
    ## Para el caso de MI COMPUTADORA <<directorio_scalogramas_test>> = 'C:\Yo\Tesis\sereData\sereData\Dataset\wavelet_cmor\test'
    ...
    create_segments_scalograms(dir_out_ov_split_test + directorio_muestra, directorio_scalogramas_test + directorio_muestra, escalas = escalas_wavelets, dt = sampling_period, actividades = act_ae)
    
    ## El parámetro <<directorio_scalogramas_test>> va a tener la ruta donde están los escalogramas que voy a usar para procesar
    ## Para el caso de MI COMPUTADORA <<directorio_scalogramas_test>> = 'C:\Yo\Tesis\sereData\sereData\Dataset\wavelet_cmor\test'
    ## El parámetro <<dir_preprocessed_data_test>> me va a almacenar los escalogramas de salida luego del preprocesado
    ## Para el caso de MI COMPUTADORA <<dir_preprocessed_data_test>> = 'C:\Yo\Tesis\sereData\sereData\Dataset\preprocessed_data\test'
    ## El parámetro <<directorio_muestra>> me va a contener la carpeta correspondiente al paciente analizado y es el que se arma en las líneas de arriba
    ## Por ejemplo, si se está analizando una muestra corta (<<long_sample>> = False) del paciente de ID 215 (<<sample_ID>> = 215)
    ## El parámetro <<act_ae>> es una lista que contiene las actividades que yo quiero procesar. Se importa del fichero <<parameters.py>> y tiene como valor <<act_ae>> = ['Caminando']
    create_or_augment_scalograms(directorio_scalogramas_test + directorio_muestra, dir_preprocessed_data_test + directorio_muestra, static_window_secs = static_window_secs, movil_window_secs = window_secs, actividades = act_ae, overlap_secs = 3, fs = fs, escalado = escalado)