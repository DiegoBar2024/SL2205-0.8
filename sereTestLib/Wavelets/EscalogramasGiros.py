...
def EscalogramasGiros(id_persona, tiempo, giros, cant_muestras, acel, gyro, periodoMuestreo):
    ...
    ## Creo una matriz donde se van a guardar los segmentos de las señales de acelerómetros y giroscopios según los giros detectados
    ...
    matriz_segmentada = np.zeros(len(giros), dtype = object)
    ...
    dimension_temporal = 800

    ## Ahora itero para cada uno de los giros que tengo segmentados
    for i in range (0, len(giros)):
        ...
        elif extension > cant_muestras - giros[i][1]:
        ...
        matriz = np.array([ acel[:,0][(giros[i][0] - extension) : (giros[i][1] + extension)],
                            acel[:,1][(giros[i][0] - extension) : (giros[i][1] + extension)],
                            acel[:,2][(giros[i][0] - extension) : (giros[i][1] + extension)],
                            gyro[:,0][(giros[i][0] - extension) : (giros[i][1] + extension)],
                            gyro[:,1][(giros[i][0] - extension) : (giros[i][1] + extension)],
                            gyro[:,2][(giros[i][0] - extension) : (giros[i][1] + extension)]])
        ...
        ## Ésto lo hago para los primeros giros en donde la extensión puede no ser completa
        extensiones_giros.append(extension)
    ...
    matriz_escalogramas = np.zeros(len(giros), dtype = object)
    ...
    for i in range (1, len(giros)):

        ## Imprimo en pantalla el número de iteración que actual
        print("Número de iteración: {} de {}".format(i, len(giros) - 1))
        ...
        matriz_escalogramas[i] = np.array([     coef1[:,extensiones_giros[i - 1]: coef1.shape[1] - extensiones_giros[i - 1]],
                                                coef2[:,extensiones_giros[i - 1]: coef2.shape[1] - extensiones_giros[i - 1]],
                                                coef3[:,extensiones_giros[i - 1]: coef3.shape[1] - extensiones_giros[i - 1]],
                                                coef4[:,extensiones_giros[i - 1]: coef4.shape[1] - extensiones_giros[i - 1]],
                                                coef5[:,extensiones_giros[i - 1]: coef5.shape[1] - extensiones_giros[i - 1]],
                                                coef6[:,extensiones_giros[i - 1]: coef6.shape[1] - extensiones_giros[i - 1]]     ])
        ...
        # data = np.abs(coef1[:,extensiones_giros[i]: coef1.shape[1] - extensiones_giros[i]])
        # cmap = plt.get_cmap('jet', 256)
        # fig = plt.figure(figsize = (5,5))
        # ax = fig.add_subplot(111)
        # t = np.arange(coef1[:,extensiones_giros[i]: coef1.shape[1] - extensiones_giros[i]].shape[1]) * periodoMuestreo
        ...
        # ## Graficación del escalograma en el plano tiempo frecuencia
        # data = np.abs(coef2[:,extensiones_giros[i]: coef2.shape[1] - extensiones_giros[i]])
        # cmap = plt.get_cmap('jet', 256)
        # fig = plt.figure(figsize = (5,5))
        # ax = fig.add_subplot(111)
        # t = np.arange(coef2[:,extensiones_giros[i]: coef2.shape[1] - extensiones_giros[i]].shape[1]) * periodoMuestreo
        # ax.pcolormesh(t, scales_freq, data, cmap = cmap, vmin = data.min(), vmax = data.max(), shading = 'auto')
        # plt.xlabel("Tiempo (s)")
        # plt.ylabel("Frecuencia (Hz)")
        ...
        # ## Graficación del escalograma en el plano tiempo frecuencia
        # data = np.abs(coef3[:,extensiones_giros[i]: coef3.shape[1] - extensiones_giros[i]])
        # cmap = plt.get_cmap('jet', 256)
        # fig = plt.figure(figsize = (5,5))
        # ax = fig.add_subplot(111)
        # t = np.arange(coef3[:,extensiones_giros[i]: coef3.shape[1] - extensiones_giros[i]].shape[1]) * periodoMuestreo
        # ax.pcolormesh(t, scales_freq, data, cmap = cmap, vmin = data.min(), vmax = data.max(), shading = 'auto')
        # plt.xlabel("Tiempo (s)")
        # plt.ylabel("Frecuencia (Hz)")
        ...
        # ## Graficación del escalograma en el plano tiempo frecuencia
        # plt.plot(tiempo[extensiones_giros[i - 1]: coef3.shape[1] - extensiones_giros[i - 1]] - tiempo[extensiones_giros[i - 1]: coef3.shape[1] - extensiones_giros[i - 1]][0], acel[:,2][extensiones_giros[i - 1]: coef3.shape[1] - extensiones_giros[i - 1]])
        # plt.xlabel("Tiempo (s)")
