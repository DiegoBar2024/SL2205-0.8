...
## Función que me construye un modelo de red neuronal fully connected arbitrario
...
def entrenamiento_clasificador(unstable_train,stable_train,unstable_validation,stable_validation,autoencoder_model,clasificador,scalogram_path_train = dir_preprocessed_data_train,scalogram_path_val= dir_preprocessed_data_test, activities=act_clf, ):
    ...
    ###############################################################
    ### FOR THE ACTIVITIES GROUP
    ###############################################################
    ## Entrenar segundo Clasificador
    ground_truth_train = np.concatenate([np.zeros(np.shape(stable_train_intermediate)[0]),np.ones(np.shape(unstable_train_intermediate)[0])]) # 0=stable 1=unstable
    ...
    if clasificador=='lda':
        #mean_unstables, std_unstables, mean_stables, std_stables, domine = plot_clf_transformation(clf_trained,unstable_validate_intermediate, stable_validate_intermediate,ground_truth_validate, labels_validate ,activities,clasificador=clasificador,extra_name=extra)
...
def train_clasificador(values,ground_truth,file_clf,clasificador, X_val=None,y_val=None ):
    ...
    ## En caso de que el clasificador sea del tipo PERCEPTRON
        ...
        keras.layers.Dense(1, activation="sigmoid", input_shape=(None,np.shape(values)[1]))])
        es = EarlyStopping(monitor='val_loss', patience=5, mode='min', verbose=1)
        clf.compile(loss="mean_squared_error", optimizer="sgd")

        #clf=Perceptron(alpha=0.003, early_stopping=True, max_iter= 100, penalty='l1', tol= 0.0001)
        clf.fit(values,ground_truth,  epochs=100, validation_data=(X_val, y_val),callbacks=[es])
    
    ## En caso de que el clasificador sea del tipo NN
        ...
        #param_distribs = {
        #    "n_hidden": [1,2,3,4],
        #    "n_neurons": [10,20,40,50,60,80,100]}

        #rnd_search_cv = RandomizedSearchCV(keras_reg, param_distribs, n_iter=10, cv=3)
        #rnd_search_cv.fit(values,ground_truth,  epochs=100, validation_data=(X_val, y_val),callbacks=[es])
    ...
    ## En caso de que el clasificador sea del tipo SVM
        ...
        #param_grid = {'C': [1, 10, 100],'gamma': [1, 0.1, 0.01],'kernel': ['linear', 'poly', 'rbf', 'sigmoid']}
    ...
    ## En caso de que el clasificador sea del tipo VOTING
        ...
        #clf3=AgglomerativeClustering(compute_full_tree=True,n_clusters=2, compute_distances=True)
        #clf4=KMeans(n_clusters=2)
        clf = VotingClassifier(estimators=[('lda', clf1), ('svc', clf2),("rf",clf3)], voting='hard')
        clf.fit(values,ground_truth)
    
    ## En caso de que el clasificador sea del tipo RF
    elif clasificador=='RF':
        #param_grid = {'n_estimators': [50, 100, 200],'min_samples_split': [ 5, 10,15],'max_depth':[3,5,10,15]}
        #gs = GridSearchCV(RandomForestClassifier(max_depth=10), param_grid, refit = True, verbose = 3,scoring='roc_auc')
        #gs.fit(values,ground_truth)
