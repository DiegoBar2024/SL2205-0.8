...
## Importación de los datos del sensor
data = numpy.genfromtxt("C:/Yo/Tesis/SL2205-0.8/SL2205-0.8/sereTestLib/Cinematica/short_walk.csv", delimiter=",", skip_header=1)
...
## Se crea un vector con la misma longitud que el vector de tiempos
is_moving = numpy.empty(len(timestamp))

## Itero para cada uno de los índices en el vector
for index in range(len(timestamp)):

    ## Afirmo que se está moviendo en caso de que la aceleración sea mayor a 3
    is_moving[index] = numpy.sqrt(acceleration[index].dot(acceleration[index])) > 3  # threshold = 3 m/s/s

## Se deja un margen de 100ms
